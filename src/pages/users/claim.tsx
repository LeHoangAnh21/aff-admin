...
import Claim from "@/libs/user/claim";
import EditClaimModal from "@/components/claim/EditClaimModal";
import AddClaimModal from "@/components/claim/AddClaimModal";
...
const ClaimPage = () => {
  ...
  const { isLoading, error, data, isFetching } = useQuery({
    queryKey: ["claims", page, lawType], // queryKey phụ thuộc vào page
    queryFn: async () => {
      const response = await Claim.list(1, localStorage.getItem("userAccount"));
      return response;
    },
  ...
  const handleAdd = useMutation({
    mutationFn: async (data) => {
      const res = await Claim.create(data, localStorage.getItem("userAccount"));
    },
    ...
    onSuccess: () => {
      ...
      // Reset form
      toast.success("Thêm hướng dẫn bồi thường thành công!", {
        autoClose: 5000,
      });
    },
    onError: (error) => {
      if (error.message.includes("already exists")) {
        toast.error("Đã tồn tại, không thể thêm mới!", {
          autoClose: 5000,
        });
      }
    },
  ...
  const handleEdit = useMutation({
    ...
    onSuccess: () => {
      ...
      // Reset form
      setPage(1);
      toast.success("Cập nhật hướng dẫn bồi thường thành công!", {
        autoClose: 5000,
      });
    },
    onError: (error) => {
  ...
  const handleDelete = useMutation({
    mutationFn: async (id) => {
      const res = await Claim.delete(id, localStorage.getItem("userAccount"));
    },
        ...
        <div className="mb-1 w-full">
          <div className="mb-4">
            <Breadcrumb className="mb-4">
              ...
              <Breadcrumb.Item>Bồi thường</Breadcrumb.Item>
            </Breadcrumb>
          ...
          <div className="sm:flex">
            ...
            <div className="ml-auto flex items-center space-x-2 sm:space-x-3">
              <AddClaimModal handleAdd={(value) => handleAdd.mutate(value)} />
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="inline-block min-w-full align-middle">
            <div className="overflow-hidden shadow">
              <ListTable
                ...
                isLoading={isLoading}
                isDisable={isDisable}
                handleUpdate={(value) => handleEdit.mutate(value)}
                handleDelete={(value) => handleDelete.mutate(value)}
              />
            </div>
          </div>
        </div>
              ...
              <Table.Cell className="whitespace-nowrap text-center p-4 text-base font-medium text-gray-900 dark:text-white">
                {" "}
                {
                  insuranceType.filter(
                    (value: any) => value.value === item.guideType
                  )[0]?.title
                }
              </Table.Cell>
              {/* <Table.Cell className="whitespace-nowrap text-center p-4 text-base font-medium text-gray-900 dark:text-white">
